...
export default function LatihanPage() {
  ...
  const exportPdf = () => {
    ...
    const html = `<html><head><title>Jadwal Latihan ${sg.namaSanggar}</title></head><body style="font-family:sans-serif;padding:32px"><h1>Jadwal Latihan — ${sg.namaSanggar}</h1><table border="1" cellpadding="6" cellspacing="0" style="border-collapse:collapse;width:100%"><thead><tr><th>Tanggal</th><th>Jam</th><th>Tempat</th><th>Kurikulum</th><th>Pelatih</th></tr></thead><tbody>${items.map(l => `<tr><td>${l.tanggal}</td><td>${l.jam}</td><td>${l.tempat}</td><td>${l.kurikulum}</td><td>${pelatihList.find(p => p.id === l.pelatihId)?.nama ?? "-"}</td></tr>`).join("")}</tbody></table></body></html>`;
