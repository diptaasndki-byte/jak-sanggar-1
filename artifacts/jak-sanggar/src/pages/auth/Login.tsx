      ...
      <div className="relative z-10 grid w-full max-w-6xl items-center gap-8 lg:grid-cols-[1.15fr_0.85fr]">
        <section className="hidden lg:flex min-h-[600px] flex-col justify-center rounded-[2rem] border border-[#15395c]/10 bg-white/25 p-10 shadow-2xl backdrop-blur-[2px]">
          ...
          <div className="mt-10 grid grid-cols-4 gap-4">
            {budayaBetawi.map((item, index) => (
              <div key={item} className="rounded-2xl border border-[#14365c]/10 bg-[#fffaf2]/75 p-4 text-center shadow-md">
                <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-[#14365c]/10 text-xl font-serif font-bold text-[#14365c]">{index + 1}</div>
                <div className="mt-3 text-xs font-semibold uppercase tracking-[0.12em] text-[#14365c]">{item}</div>
              </div>
            ))}
          </div>
